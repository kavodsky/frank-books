"""SMART lemma arbitration is cached on the disputed-type set (roadmap 2.2b)."""
...
from frank.domain.model.lemma import DisputedLemma, LemmaSource
...
def test_unchanged_disputes_make_zero_llm_calls_on_rerun(tmp_path) -> None:
    hits = {"n": 0}
    body = json.dumps(
        {
            "items": [
                {"surface": "sah", "upos": "VERB", "lemma": "sehen"},
            ]
        }
    )
    ...
    arbiter = SmartLemmaArbiter(
        ...
        lexicon=_Lexicon(frozenset({"sehen"})),
    )
    disputed = (
        DisputedLemma(
            surface="sah",
    ...
    first = arbiter.decide(disputed)
    assert hits["n"] == 1
    assert first[0].lemma == "sehen"
    assert first[0].source is LemmaSource.LLM
    second = arbiter.decide(disputed)
    assert hits["n"] == 1
    assert second == first
